...
char* stringdup(const char* s, size_t len)
{
    ...
    long l = (len==STRINGDUP_NOLEN)?strlen(s):len;
...
WCHAR* wstrdup(const WCHAR* s, size_t len)
{
    ...
    long l = (len==STRINGDUP_NOLEN)?wcslen(s):len;
...
char* MD5CredentialData(const char* userName, const char* password, const char* nonce) {
    ...
    long len = 0, lenNonce = 0, totLen = 0;
    ...
    // H(username:password)
    calculateMD5((void*)token, (int)len, digest);
    ...
    memset(digest, 0, 16);
    calculateMD5(base64Nonce, (int)totLen, digest);
