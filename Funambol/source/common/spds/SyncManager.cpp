...
{
    LOG.debug("calling method:%s",__FUNCTION__);
    char *key;
...
void SyncManager::encodeItemKey(SyncItem *syncItem)
{
    ...
    if (wcschr(syncItem->getKey(), '<') || wcschr(syncItem->getKey(), '&')) {
        ...
        b64_encode(encoded, key, (int)strlen(key));
...
void SyncManager::initialize() {
    ...
    maxMsgSize   = (int)config.getMaxMsgSize();
    if (maxMsgSize <= 0) {
    ...
    if (config.getReadBufferSize() > 0)
        readBufferSize = (int)config.getReadBufferSize();
...
int SyncManager::prepareSync(SyncSource** s) {
    ...
    fireSyncEvent("sync begin", SYNC_BEGIN);
...
//    mmanager[count] = 0;
    ...
    if (config.getSendDevInfo()) { 
        ...
        if (devInf) {
            ...
            devInfStr->append("</SyncURL>");
            calculateMD5(devInfStr->c_str(), (int)devInfStr->length(), md5);
    ...
    do {
        ...
        if (isClientAuthenticated == false) {
            ...
            const char *password = cred->getAuthentication()->getPassword();
            if (password) {
                strcpy(credentialInfo, password);
            }
        }
        ...
        fireSyncEvent("send initialization", SEND_INITIALIZATION);
    ...
    while (sortedSourcesFromServer[i]) {
        ...
        if (sync) {
            ...
            if (isFiredSyncEventBEGIN == false) {
                ...
                fireSyncSourceEvent(sources[count]->getConfig().getURI(),
									sources[count]->getConfig().getName(),
									sources[count]->getSyncMode(), (int)noc, SYNC_SOURCE_TOTAL_SERVER_ITEMS);
            ...
            for (int i = 0; i < items->size(); i++) {
                ...
                for (int j = 0; j < list->size(); j++) {
                    ...
                    if (status) {
//Delete:                        syncMLBuilder.addItemStatus(&previousStatus, status);
                        deleteStatus(&status);
                    }
                }
...
//            }
                ...
                case SYNC_REFRESH_FROM_CLIENT:
				{
					...
					do {
						...
						if (syncItem) {                                                                
							...
							// extra safe check...
							if (isTooBig((int)chunk->getDataSize(), maxMsgSize, msgSize)) {
            ...
            fireSyncEvent("send modification", SEND_MODIFICATION);
...
//            mmanager[count]->closeMappings();
    ...
    if (msgToSend) {
        ...
        fireSyncEvent("send finalization", SEND_FINALIZATION);
...
//        }
    ...
    // Fire Sync End Event
    fireSyncEvent("sync end", SYNC_END);
...
SyncItem* SyncManager::getItem(SyncSource& source, SyncItem* (SyncSource::* getItemFunction)()) {
	...
	//printf("syncItem to server:%s\n",(char*)syncItem->getData());
    return syncItem;
}
...
Status *SyncManager::processSyncItem(Item* item, const CommandInfo &cmdInfo, SyncMLBuilder &syncMLBuilder)
{
    ...
    if (incomingItem) {
        ...
        // if (incomingItem->cmdName.c_str() && strcmp(incomingItem->cmdName.c_str(), cmdInfo.commandName) || count != incomingItem->sourceIndex)
...
//                        WCHAR* tparent = toWideChar(parentLUID.c_str());
                        WCHAR* tparent = toWideChar(parentGUID.c_str());
